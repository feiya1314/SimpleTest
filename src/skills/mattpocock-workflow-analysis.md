...
1. **TDD 循环**：让 AI 先写测试
2. **两轴 Code Review**：提交前做 Standards + Spec 审查
3. **Seam 确认**：写代码前确认在哪里测试

### 第三阶段：处理大型工作

1. **Wayfinder**：面对模糊的大任务，先画地图再行动
2. **To-Tickets**：把 Spec 拆成可执行的 tickets
3. **Handoff**：跨会话交接

### 第四阶段：架构演进

1. **深度模块设计**：用 deep module 语言设计接口
2. **Domain Modeling**：持续精炼术语和记录架构决策
3. **Improve Codebase Architecture**：定期运行，防止代码变成"泥球"
...
架构决策记录（ADR）是一份文档，其中描述了团队针对他们计划构建的软件架构的重要方面所做的选择。 每个ADR 都描述了架构决策、上下文以及结果。 ADR 有状态，因此它会遵循生命周期。它是一份轻量文档,每做一个重要架构决策,就写一份,记下这个决策的来龙去脉。它是一份**轻量**文档,每做一个重要架构决策,就写一份,记下这个决策的来龙去脉。

它的精髓在于足够轻:

- **一个决策一份**,通常一页纸,几分钟能读完。
- **追加,不修改**:决策变了就写新的一份,把旧的标记为被取代——**保留历史,而不是抹掉历史**。因为团队曾经怎么想、后来为什么改,本身就是宝贵信息。
- **和代码放在一起**:例如仓库里一个 `docs/adr/` 目录,让它随代码一起被版本管理、被搜索、被 review。
