...
## 2. Producer 消息写入流程是怎样的？

1. 找到Partition Leader
2. Producer 确定目标 partition
3. 从 Zookeeper 的 `/brokers/.../state` 节点找到该 partition 的 **leader**
4. Producer 只与 leader 通信，将消息发送给 leader
5. leader 将消息写入本地 log
6. followers 从 leader **pull** 消息，写入本地 log **后 leader 发送 ACK**
7. leader 收到**所有 ISR 中的 replica 的 ACK** 后，增加 HW（High Watermark，最后 commit 的 offset），并向 Producer 发送 ACK
...
- 配置：`acks=0`
- Producer 发送完消息后不确认是否成功送达，存在丢失可能性 （生产者没有等待 ack，它以为发送成功； **broker 宕机重启后这条消息消失丢失** ）
- 生产者**不等待 ack，不知道发送成功还是失败**，**不会发起第二次发送**。只要生产者不重发，自然不存在重复。

**At least once（消息不会丢，但可能重复传输）**

- 配置：`acks=1`（默认），`retries=2147483647`
- Producer 确认消息是否发送成功，未收到 ACK 则不断重试，消息可能被发送多次
- **Broker 写入成功，ACK 丢了** ：**最容易产生重复**
...
- 生产者分配 **PID（生产者 ID）**，每条消息自带 **SequenceNumber 序列号**。broker 通过 PID + 序列号识别这条消息已经持久化，**直接丢弃重复请求**。
...
一个 Topic 的一个 Partition 在物理上对应一个文件夹，文件夹下由多组 **Segment 文件**组成，每组包含 `.index` 文件和 `.log` 文件。  **log文件就实际是存储message的地方** ，而index和timeindex文件为索引文件，用于检索消息。
...
- Linux 内核利用空闲内存缓存磁盘文件内容的机制，以**页（page）**为单位缓存文件数据（页大小通常为 **4KB**）
- **读文件**：内核先查 page cache，**命中**则直接从内存返回，不访问磁盘；**未命中**才从磁盘读入并缓存，后续再读可直接命中
- **写文件**：采用 **write-back（回写）机制**，数据先写入 page cache 并标记为 dirty，由内核后台线程**异步批量刷盘**，避免每次写都落盘

**Kafka 与 Page cache**

- Kafka 的日志数据**不缓存到 JVM 堆内**，而是直接利用操作系统 page cache
- 只要**消费速度跟得上生产速度**，数据从 Producer 写入后一直留在 page cache 中，Consumer 直接从 page cache 读取，**消费不产生磁盘读**，这是 Kafka 高吞吐的重要原因
- page cache 是**全局共享且容量有限**的，内存不足时内核会按 LRU 等策略淘汰不常用的页
...
- **缓存页资源不足**，底层数据被主动替换。消费速度跟不上生产速度时，数据无法保留在 page cache 中，**缓存命中率持续下降、冷数据反复挤占热数据（缓存颠簸）**，导致频繁磁盘 I/O

**如何避免**

1. **保证消费速度跟上生产速度**：只要数据能留在 page cache 中，就不会被淘汰，这是最根本的解决方式
2. **扩容消费端**：增加同消费组的 Consumer 实例数量，提高并行消费能力（Consumer 数量不要超过分区数，否则多余实例闲置）
3. **增加分区数并配套增加消费者**：单纯加分区不提升消费并行度，需同时增加 Consumer 实例数才有效
4. **优化消费逻辑**：增大单次拉取量（如 `max.partition.fetch.bytes`），采用批量、异步处理，提升消费吞吐
5. **合理分配内存**：Kafka 自身堆内缓存极少，应**控制 JVM 堆大小（通常 4\~8GB 足够），把尽量多的物理内存留给 OS page cache**
6. **隔离污染源**：把消费慢、长期无人消费的 topic 放到独立 Broker 或机器上，避免其冷数据挤占正常 partition 的热缓存
7. **及时清理无用数据**：对可丢弃的日志类数据及时消费或调低 `log.retention`，避免无限堆积变成持续的污染源
8. **监控消费滞后（Lag）并告警**：一旦发现消费跟不上生产，及时扩容或优化，避免污染持续恶化
