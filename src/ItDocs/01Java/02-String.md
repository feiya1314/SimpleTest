...
**演化关系（运行时常量池 vs 字符串常量池）：**

- JDK6及之前：运行时常量池**逻辑包含**字符串常量池，都存 **放在方法区（永久代实现，位于堆内存中** ）Perm 区是一个类静态的区域，主要存储一些加载类的信息，常量池，方法片段等内容
- JDK7：字符串常量池从方法区**移到了堆中**， **运行时常量池剩余内容继续留在方法区（** 永久代）
- JDK8：永久代被移除，改为**元空间**（堆外本地内存）， **运行时常量池仍在方法区（元空间实现）** ， **字符串常量池仍在堆**

**核心结论：** 运行时常量池**一直在方法区（Non-heap）**，JDK7后字符串常量池被移到了**heap区**，两者**根本不是一个概念**。

**关键区分：**

- **Class常量池**：每个class文件都有一个常量池，存放**字符串常量**、**类和接口全限定名**、**字段名描述符**、**方法名描述符**等在class文件中引用的常量
- **运行时常量池**：在方法区（JDK8及以后为元空间），类加载时从Class常量池解析而来，存放类版本、字段、方法、接口等描述信息
- **字符串常量池**：JDK7前在方法区（Perm），JDK7后在堆中，专门**缓存字符串对象**以减少内存开销
