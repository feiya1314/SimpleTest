...
- **公平锁**：先到的线程优先获取锁，按 FIFO 顺序，避免线程长期处于饥饿状态
...
1. `volatile state`：保证锁状态在多线程间的可见性、禁止指令重排；
2. **Unsafe CAS 原子操作**：原子更新 state、队列头尾节点，解决并发修改竞态；
3. **CLH 等待队列 + LockSupport (park/unpark)**：管理竞争失败线程，实现阻塞与唤醒，避免持续自旋。
...
1. `tryAcquire(arg)` - 尝试获取锁（子类实现），一般是判断 state 是否为0，为0则 **CAS修改 state**
...
**原理分析**
![Lock接口层次图](../assets/01Java/11aad92e96574d45b91c29238bfd1f4e.png)
**1. lock()**
...
![重量级锁开销](../assets/01Java/cfbf09fb7b5344e78194fb9957acf9f7.png)

![锁状态转换](../assets/01Java/0eb4d668c6834c92906c1e0c026bb856.png)
...
1. **减少持有锁时间**：缩小同步块范围
2. **使用 Lock 替代 synchronized**：tryLock 可超时回退
...
# 15. synchronized与ReentrantLock的对比

synchronized 和 ReentrantLock 有哪些区别？如何选择？
**原理分析**

| 对比维度 | synchronized | ReentrantLock |
| --- | --- | --- |
| 锁灵活性 | 自动获取和释放锁（退出同步块自动释放），使用简单但不够灵活 | 需要手动 lock/unlock，通常结合 try/finally 释放锁，灵活性更高 |
| 等待可中断 | 等待锁时**不可中断**，线程会一直阻塞直到获取锁 | 支持**可中断等待**，通过 `lockInterruptibly()` 可在等待时响应中断 |
| 公平性 | **非公平锁**，不支持公平模式 | 默认非公平，可通过构造函数 `new ReentrantLock(true)` 开启公平模式 |
| 条件变量 | 只能关联**一个** waitSet（通过 Object.wait/notify） | 可创建**多个** Condition 对象，实现更精细的线程通知控制 |
| 超时获取 | 不支持超时等待 | 支持 `tryLock(long, TimeUnit)` 超时获取锁 |
| 性能对比 | JDK6 优化后，**synchronized 和 ReentrantLock 性能基本持平**<br>synchronized 偏向锁、轻量级锁在**低竞争**场景更有优势 | ReentrantLock 在**高竞争**场景略优，但差距已经很小 |
| 底层实现 | JVM 层面实现，基于 monitor 对象，锁升级过程由 JVM 管理 | JDK 层面实现，基于 **AQS + CAS + LockSupport** |

**如何选择：**

- 能用 synchronized 就用 synchronized：代码更简洁，不易出错，JDK 持续优化中
- 需要**可中断等待、超时获取、公平锁、多个条件变量**等高级功能时使用 ReentrantLock
