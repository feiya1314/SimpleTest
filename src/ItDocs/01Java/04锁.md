...
3. 支持**锁降级**（写锁→读锁），不支持锁升级（读锁→写锁）
   ...
   **解决方案：**
4. **JDK8 StampedLock**：读不阻塞写，发生写时重读
5. **使用公平锁**：`new ReentrantReadWriteLock(true)` 按 FIFO 分配
...
- **synchronized**：自动获取和释放锁（退出同步块自动释放），使用简单但不够灵活
- **ReentrantLock**：需要手动 lock/unlock，通常结合 try/finally 释放锁，灵活性更高
  **2. 等待可中断**
- **synchronized**：等待锁时**不可中断**，线程会一直阻塞直到获取锁
- **ReentrantLock**：支持**可中断等待**，通过 `lockInterruptibly()` 可在等待时响应中断
  **3. 公平性**
- **synchronized**：**非公平锁**，不支持公平模式
- **ReentrantLock**：默认非公平，可通过构造函数 `new ReentrantLock(true)` 开启公平模式
  **4. 条件变量**
- **synchronized**：只能关联**一个** waitSet（通过 Object.wait/notify）
- **ReentrantLock**：可创建**多个** Condition 对象，实现更精细的线程通知控制
  **5. 超时获取**
- **synchronized**：不支持超时等待
- **ReentrantLock**：支持 `tryLock(long, TimeUnit)` 超时获取锁
  **6. 性能对比**
- JDK6 优化后，**synchronized 和 ReentrantLock 性能基本持平**
- synchronized 偏向锁、轻量级锁在**低竞争**场景更有优势
- ReentrantLock 在**高竞争**场景略优，但差距已经很小
  **7. 底层实现**
- **synchronized**：JVM 层面实现，基于 monitor 对象，锁升级过程由 JVM 管理
- **ReentrantLock**：JDK 层面实现，基于 **AQS + CAS + LockSupport**
  **如何选择：**
