...
**定义：** 线程池中Worker线程执行任务时抛出RuntimeException或Error未被捕获，导致线程退出runWorker循环，线程池大小永久减1，最终线程池为空，系统无可用线程。

**原因：**

- 任务代码没有try-catch包裹，抛出未检查异常
- 异常导致runWorker循环提前退出，Worker从workers集合中移除
- 线程池不会自动补充因异常退出的线程（除非后续有新任务提交触发addWorker）

**如何防范：**

1. **任务内部自行try-catch**：每个任务的run方法包裹try-catch，确保异常不逃逸
2. **设置UncaughtExceptionHandler**：通过ThreadFactory设置自定义UncaughtExceptionHandler，记录异常日志
3. **使用submit()替代execute()**：submit()提交的任务，异常会被封装在Future中，调用future.get()时抛出，不会直接导致线程退出
4. **包装ThreadPoolExecutor**：重写afterExecute方法，在方法中捕获并处理异常
