...
**定义：** 线程池中Worker线程执行任务时抛出RuntimeException或Error未被捕获，导致该Worker退出runWorker循环，workerCount减1。

**原因：**

- 任务代码没有try-catch包裹，抛出未检查异常
- 异常导致runWorker循环提前退出，Worker从workers集合中移除
- 触发`processWorkerExit`方法销毁Worker，但该方法同时会尝试调用`addWorker(null, false)`来**自动补充一个新Worker**，一般不会导致线程池为空
- 但如果在SHUTDOWN/STOP状态下，`addWorker`会失败，新Worker无法创建，此时线程池才会逐渐萎缩
