...
一个完整的提示词，通常由**六个核心要素**组成：**角色（Role）、任务（Task）、上下文（Context）、输入数据、输出格式（Output）、示例（Example / Few-Shot）**。下面逐一拆解。
...
买来一个月了，音质确实可以，高音部分很清晰。戴久了耳朵有点疼，舒适度一般。电量能撑一整天，续航没问题。就是价格偏贵，性价比不高。
```
...
要求 AI **「一步一步地思考」**，把推理过程显式地走一遍，能**显著提升逻辑推理和数学计算的准确率**。因为直接要结果时，模型容易跳步、出错；让它逐步推理，中间步骤出错时也更容易被定位和纠正。
...
逐步提问（CoT）：
第一步：记录初始状态——5 个苹果、3 个梨
第二步：妈妈拿走 2 个苹果——剩 3 个苹果、3 个梨
第三步：爸爸给了 1 个梨——3 个苹果、4 个梨
第四步：拿 1 个梨和姐姐换 1 个苹果——4 个苹果、3 个梨
最终答案：4 个苹果、3 个梨
```
...
CoT 是针对**逻辑比较强、对准确性要求高**的任务提出的，核心是**提醒或约束模型按要求的思考路径走**，以提升准确率。在复杂场景下，CoT 也可以理解为**执行流程或思考过程**，作为整个 Prompt 的一部分：模型在充分理解任务和上下文之后，再按 CoT 步骤执行拆解任务，**「听话」程度会高很多**。在逻辑、数学等推理类任务上，CoT 通常能带来明显的准确率提升（具体幅度因任务和模型而异）。
...
**上下文工程（Context Engineering）**，核心定义一句话：**构建一个动态系统，以正确格式提供正确的信息和工具，从而使大语言模型（LLM）能够可靠地完成指定任务**。

相当一部分人会认为用户的历史聊天记录就是上下文。这个理解是部分正确的，上下文所涵盖的范围要远大于简单的历史对话记录。从本质上讲 **，上下文（Context）是提供给 LLM 的、用于完成下一步推理或生成任务的全部信息集合。**

它被称作"Agentic System 开发之禅"。一句话点明它的分量：**多数 AI Agent 的失败，并非模型能力的失败，而是上下文工程的失败**。

## 17.1 提示词工程 vs 上下文工程

先建立直觉，两者目标不同：

- **提示词工程（Prompt Engineering）**：目标是**写出清晰、明确的指令**。提示词本质是布置任务的指令，让 AI 按预期方式回答问题、执行任务
- **上下文工程（Context Engineering）**：目标是**准备好一切 AI 该知道的信息**。上下文本质是告诉 AI 当前需要什么、能做什么，提供所需的信息、工具和历史记忆，然后交给 AI 思考和执行

**一句话总结两者关系：**

- 上下文负责**铺路**，提示工程教你怎么**走**
- **提示工程是上下文工程的子集**
- **上下文工程 = 提示词 + 用户画像 + 记忆 + 检索信息 + RAG 信息 + MCP 信息**

**为什么从"提示词艺术"转向"上下文科学"：**

- 提示词工程依赖**精细指令**：难以复用，容易因措辞差异导致结果波动，对写提示词的人要求也更高
- 上下文工程让 AI 具备**持续理解和应变能力**：不仅能适应新任务，还能保持输出稳定、不易"翻车"
- 大模型应用技术正从"提示词艺术"迈向"上下文科学"。Cursor、Manus 等爆品表明：一个卓越的大模型产品，背后必然是精心设计的上下文工程系统。**未来的核心竞争力，不再是精妙的提示词，而是高质量、高效率的上下文**

## 17.2 为什么需要：智能体不稳定的根源

使用 Agent 时常常遇到棘手问题： **智能体为什么这么不稳定？** 很多人归咎于模型本身不够强大。**然而问题的根源往往是：未能向模型提供恰当的上下文**——没有以正确格式向模型传递正确的信息、指令和工具。

**Cognition 团队核心观点**：再聪明的模型，若不知上下文，也无法做出正确判断。智能体系统的失误，本质是 LLM 失误。

**从第一性原理分析，LLM 失败主要源于两个方面：**

1. **底层模型能力不足**：无法正确推理或执行
2. **模型缺少正确判断所需的上下文**：随着模型能力飞速发展，第二点成为更普遍的瓶颈

**上下文供给不足体现在两点：**

- **信息缺失**：模型完成任务所依赖的关键信息未被提供，Agent 未能推进任务，因为 **它缺乏做出下一步决策所需的信息性上下文。**
- **格式混乱**：信息存在，但其组织与呈现方式不佳，阻碍了模型的有效理解

**三个关键原则：**

- **垃圾进，垃圾出**（Garbage in, garbage out）：LLM 无法读取用户思想，如果不提供完成任务所需的关键信息，它不可能知道这些信息的存在；信息质量直接决定输出质量
- **工具与信息同等重要**：很多场景下 LLM 需要借助工具查询更多信息或执行动作，提供正确的工具与提供正确的信息同等重要
- **格式化至关重要**：与人类沟通类似，如何向 LLM 传递信息的格式会极大影响结果。一个简短但描述清晰的错误信息，远比一个庞大而杂乱的 JSON 数据块更容易让 LLM 理解和处理；工具输入参数是否清晰、易于理解，也直接决定 LLM 能否有效使用

## 17.3 核心定义与深入理解

**定义**：构建一个动态系统，以正确格式提供正确的信息和工具，从而使大语言模型（LLM）能够可靠地完成指定任务。

**深入理解六个要点：**

- **系统工程**：复杂智能体从多个来源获取上下文（开发者预设、用户输入、历史交互、工具调用结果、外部数据），将这些信息源有机整合起来，本身就是一个复杂的系统工程
- **动态变化**：上下文很多组成部分是实时生成的，最终输入给模型的提示（Prompt）也必须是动态构建的，而非静态模板
- **信息精准**：智能体系统失败的原因之一是上下文信息缺失，LLM 无法"读心"，必须为其提供完成任务所需的全部信息；垃圾进、垃圾出
- **工具适用**：很多任务无法仅靠初始信息完成，为 LLM 配备合适的工具至关重要（用于信息检索、执行动作或与其他系统交互）
- **格式规范**：与 LLM 的"沟通方式"跟人一样关键，简洁明了的错误信息远胜于一个庞杂的 JSON 数据块；工具参数设计、数据呈现格式都会显著影响 LLM 的理解和使用效率
- **任务可行性**：设计系统时反复自问"在当前提供的上下文和工具下，LLM 是否真的有可能完成任务？"，便于归因分析——是上下文不足，还是模型本身的执行失误，从而采取不同的优化策略

**一个形象类比：LLM as OS**——如果把 LLM（或更广义的 Agentic System）视为一种新型操作系统：

- LLM 就像 **CPU**
- 上下文窗口（Context Window）就像 **RAM**（同样是容量有限，是管理 CPU 所需要的内存）
- **上下文工程就是内存管理器**：它的职责不是简单地把数据塞满 RAM，而是通过复杂的调度，决定在每一个时刻哪些数据应该被加载、哪些应该被换出、哪些应该被优先处理，从而保证整个系统的流畅运行和最终结果的准确性

这正对应了那句定义：**上下文工程是一门微妙的艺术与科学，旨在在上下文窗口中填入恰到好处的信息，为下一步推理做准备**。它标志着我们与 LLM 的交互模式，从原来的"优化提示词"升级为"构建最高效的信息供给系统"。

## 17.4 上下文的三分类

上下文（Context）是**提供给 LLM 的、用于完成下一步推理或生成任务的全部信息集合**。它远不止历史聊天记录，可划分为三个核心类别：

**指导性上下文（Guiding Context）**——指导模型该做什么、如何做，主要为模型行为设定框架、目标和规则：
...
**信息性上下文（Informational Context）**——告诉模型需要知道什么知识，为模型提供解决问题所必备的事实、数据与知识：

- RAG（外部知识库检索）
- Memory（记忆）：短期记忆（当前对话历史）、长期记忆（跨会话存储的重要信息和偏好）
- State / Scratchpad（状态维护和草稿纸，如规划时的临时 TODO、思考模型的"草稿本"）

**行动性上下文（Actionable Context）**——告诉模型能做什么、做了之后的结果，为模型提供与外部世界交互的能力：

- Tool Definition（工具定义）
- Tool Calls & Results / Tool Traces（调用了哪些工具、工具返回的结果，即工具调用的历史追踪）

**总结**：上下文是一个**多维、动态、服务于特定任务的系统性概念**。

## 17.5 上下文退化：为什么不能简单堆砌

一个直觉方案是"把所有可能的上下文都提供给模型"。但暂且不考虑模型能力和经济成本，当任务变得复杂、长程时，**"简单累加"策略不仅会失效，甚至会降低模型表现**，产生三阶段失败级联：

1. **性能下降**：早期任务中无关紧要的细节（如一次已解决的语法错误）不断稀释当前步骤所需的核心信息，造成**上下文干扰（Context Distraction）**
2. **成本与延迟激增**：上下文线性增长，每次 API 调用的 Token 数量急剧膨胀，导致成本失控、延迟升高
3. **最终崩溃**：累计信息总量超过上下文窗口上限（无论是 200K 还是 1M tokens），API 调用直接失败，任务中断或因信息截断而引向错误的结果

**结论**：上下文是**需要被主动管理的、有限的资源**，简单的累加策略在规模化应用中不可持续。

**长上下文暴露的四类上下文失效：**

- **上下文污染（Context Poisoning）**：幻觉进入上下文，导致异常结果
- **上下文干扰（Context Distraction）**：上下文接近溢出时，模型训练中获得的知识被"覆盖"，导致模型降智
- **上下文混淆（Context Confusion）**：冗余且不相关的上下文让输出结果偏离期望
- **上下文冲突（Context Clash）**：上下文中的信息互相矛盾（如上下文存在过去错误的答案）

这四类失效常彼此耦合，直接损害推理稳定性与跨代理传递。

塞给它越多无关信息，它的注意力就越散， **出现上下文腐化 。** 也就是让 Agent 边干活边按需抓信息，而不是一上来就把所有可能有用的东西一股脑塞进去。

## 17.6 最佳实践：写、选、压、隔

业界已逐步收敛出一套系统性的应对框架，上下文工程分为四个操作：**写入（Write）、选取（Select）、压缩（Compress）、隔离（Isolate）**。

### 写入（Write）

写入指**将上下文持续持久化，超越上下文窗口的限制，在未来按需取用**。通常分为：

- **会话内写入（Session-level Write）**：Agent 将中间思考、计划或临时数据写入会话内的草稿纸（Scratchpad）。这是一种轻量级、非持久化的写入，用于管理当前任务的复杂性
- **持久化写入（Persistent Write）**：系统将具有长期价值的信息（如用户偏好总结、关键事实）写入外部记忆系统（如向量数据库、知识图谱），实现跨会话的知识积累。例如 ChatGPT、Cursor 等应用通过这种方式，让 Agent 在持续交互中"学习"和"成长"
- **实践建议**：Anthropic 建议把 Subagent 的输出写到文件系统，避免"传话游戏"式的无意义上下文传递，减少上下文窗口占用

### 选取（Select）

选取指**在每次 LLM 调用前，从所有可用的信息源中动态拉取与当前子任务最相关的信息**，这是保证上下文信噪比的关键。包括三类：

- **确定性选取（Deterministic Select）**：按预设规则加载上下文。例如编码 Agent（Claude Code）启动时固定加载项目根目录下的 CLAUDE.md，是一种简单高效的先验知识注入
- **模型驱动选取（Model-driven Select）**：可用信息源过多（如海量工具或文档）时，利用模型自身的能力进行筛选
- **检索式选取（Retrieval-based Select）**：最主流的方式，通过相似度检索从记忆、草稿纸或外部知识库中选取信息，成败在很大程度上依赖底层检索系统的质量

### 压缩（Compress）

压缩指**在信息进入上下文窗口之前，对其进行有损或无损压缩，用更少的 Token 承载最核心的信号**。这是在上下文窗口容量有限的情况下容纳更多有效信息的直接手段：

- **自动压缩（auto-compact）**：上下文接近溢出时自动总结上下文、保留"它认为最重要的部分"，例如 Claude Code 的 auto-compact 功能
- **专用压缩模型**：Cognition AI 为高效地在 agent 之间传递上下文，使用了微调过的"压缩大语言模型"专门执行压缩步骤
- **修剪策略**：如硬截断超限历史，代价是失去部分语境

### 隔离（Isolate）

与前三个在"信息流"内部进行优化的原则不同，隔离是**在系统架构层面进行、更根本的上下文管理策略**：在多信息流之间设置边界，由子流程先行消化、仅上交要点，可视为跨流层面的"压缩"：

- **子智能体 = 智能过滤器**：子智能体在各自领域内隔离且并行工作，消化大量原始信息，然后将最关键的"压缩后"的洞见（the most important tokens）提交给主智能体
- **减轻主智能体认知负担**：主智能体无需亲自阅读每一份原始文档，只需处理各专家团队提交的、经过预处理和提炼的"摘要报告"
- 经典表现是**多智能体架构**，可避免上下文干扰/冲突，侧面提高上下文信息密度；Tool call 以及类似的沙盒环境也体现同等的隔离思想

**压缩 vs 隔离：**

- **压缩**作用于单一信息流的内容，旨在提升其内在的信息密度
- **隔离**作用在多条信息流上，旨在管理系统的复杂性，同时也能起到广义的压缩作用
- 两者殊途同归，最终都是为了提升 Context 中的信息密度，一个成熟的系统往往会同时运用这两种策略

## 17.7 上下文工程与 Prompt / RAG / MCP 的关系

三者不是排斥关系，而是**处于不同层级、互相协作**的关系：

- **提示词工程是上下文工程的子集**：主要优化指导性上下文（Guiding Context），是更细粒度、面向单轮交互的工程实践
- **RAG 是实现动态信息性上下文的关键技术手段之一**：从外部知识库检索相关信息，作为信息性上下文的一部分填充到上下文窗口。上下文工程的范畴远大于 RAG——它不仅负责"检索什么"，还需考虑如何将信息性上下文与另外两类上下文进行动态组合
- **MCP（Model Context Protocol，模型上下文协议）是实现稳健上下文工程的基础设施**：面向工具与数据的标准化接口，本质是"行动性上下文"和部分"信息性上下文"的标准化交互，让 Agent 能够更顺畅、更安全地与外部工具和数据源进行通信

**一句话收尾**：无论精巧的 Prompt、RAG 还是 MCP，都指向同一个目标——在模型做出决策前，为它准备好一份恰到好处的上下文。**多数 AI Agent 的失败，并非模型能力的失败，而是上下文工程的失败。**

## 17.8 面试总结

回答「上下文工程」这道题，按三个层次组织：

**第一层：先讲定义与关系**——上下文工程是构建动态系统，以正确格式提供正确的信息和工具，使 LLM 可靠完成任务。一句话对比：提示工程是"怎么做"，上下文工程是"准备做成事的条件"；**提示工程是上下文工程的子集**；上下文工程 = 提示词 + 用户画像 + 记忆 + 检索信息 + RAG 信息 + MCP 信息。

**第二层：讲为什么（智能体不稳定的根源）**——再聪明的模型，若不知上下文也无法做出正确判断。LLM 失败两大来源：模型能力不足、上下文缺失；模型能力提升后，后者成为更普遍瓶颈。强调三个原则：垃圾进垃圾出、信息与工具同等重要、格式化至关重要。

**第三层：讲怎么做（分类 + 四大原则）**——上下文分三类：指导性（System Prompt 等）、信息性（RAG、记忆、草稿纸）、行动性（工具定义与调用结果）。最佳实践四原则：写入（会话内/持久化）、选取（确定性/模型驱动/检索式）、压缩（auto-compact/压缩模型）、隔离（多智能体、子代理过滤器）。最后补充与 RAG、MCP 的关系，收尾点出"多数 Agent 的失败是上下文工程的失败"。

# 18. Harness驾驭工程

**Harness Engineering（驾驭工程 / 马具工程）**，核心定义一句话：**除了 LLM 模型本身之外，让 Agent 真正能干活的一切基础设施**。它不是"更好的提示词"，也不是"更强的模型"，而是**优化模型运行环境、机制与基础设施的总和**——一套把 AI 的"智能"转化为可靠、可控、可规模化的"生产力"的工程哲学与实践框架。

把"AI 该怎么干活"固化成可执行、可约束、可评测的工程框架。

## 18.1 起源与核心概念

### 18.1.1 核心隐喻：马与缰绳

- 模型 = 强大但方向不定的**野马**；Harness = 约束、引导并纠正其行为的**缰绳（马具）**
- 通过约束、引导并纠正行为，确保 Agent 沿预设轨道**稳定、可靠地前行**
- **AI Agent = SOTA 模型（野马） + Harness（驾驭系统） = 千里马**
- Harness 不改变"马的基因"（模型本身），而是为它设计一整套"马具与训练方法"（运行环境与机制）

### 18.1.2 核心等式

- **Agent = Model + Harness**：Agent 系统中，除了模型本身之外，几乎所有决定它能否稳定交付的东西都属于 Harness
- **Harness = Agent − Model**：用减法划清边界——Prompt、上下文管理、工具、编排、记忆、评估、约束、恢复等全部属于 Harness

### 18.1.3 解决的核心问题

当 AI 成为团队一员（"超级实习生"）时，**如何管理它**：如何在真实执行中让它**持续做对一连串的事**，而不是单轮"答对"； **出了问题还能自己爬起来。**

## 18.2 为什么需要 Harness Engineering

### 18.2.1 构建可靠 Agent 的四个核心目标：R.E.S.T 模型

为了让 Agent 从"有趣的玩具"变为"可靠的工具"，必须满足四个核心目标：

1. **可靠性（Reliability）**——面对预期/非预期输入、环境变化和内部故障时，持续稳定提供服务：
   - **失败可恢复**：任务中断后能自动从检查点恢复
   - **操作幂等性**：关键写操作可安全重试，不弄脏状态
   - **行为一致性**：相同输入下行为可预测
2. **效率（Efficiency）**——资源使用有效性，直接关系成本与可扩展性：
   - **资源可控**：对 Token 消耗、API 调用、计算时间有精确预算控制
   - **低延迟响应**：交互式场景快速给出有意义反馈
   - **高吞吐量**：批处理场景单位时间处理更多任务
3. **安全性（Security）**——保护系统及数据免受未授权访问、使用、泄露或破坏（自主 Agent 不可逾越的红线）：
   - **最小权限**：仅授予完成当前子任务所必需的权限
   - **沙盒执行**：所有不受信任的代码或指令在严格隔离的沙盒中执行
   - **输入/输出过滤**：防止指令注入、敏感信息泄露和有害内容生成
4. **可观测性（Traceability）**——提供足够数据（日志、指标、追踪），使开发和运维理解内部状态、决策过程和行为轨迹：
   - **全链路追踪**：从请求到结果的每一环节调用链清晰可追溯
   - **决策可解释**：关键决策（如选择哪个工具）有明确归因记录
   - **状态可审计**：任意历史时间点的完整状态可查询和审计

### 18.2.2 Agent-First 时代对工程师的必然要求

- **工程复杂性持续攀升**：应用场景从 Vibe Coding 小 Demo 跃迁为复杂工程实践
- **角色跃迁：从"执行者"到"设计者"**——AI 承担代码编写后，人类工程师从"逐行编码的工人"变为"设计蓝图、定义规则、验收成果的架构师"
- 仅靠给 AI 制定 Prompt 规则这种"软约束"远远不够，需要一套"硬约束"的工程体系来保障最终产物的**质量、可靠性与可维护性**

### 18.2.3 核心理念

**当模型遇到问题时，通过一套工程化的 Harness 机制，从根本上避免同类问题再次发生。**

- 它是动态演进的：随着模型迭代，更多基础能力会被内化到模型本身，部分 Harness 会退出历史舞台；同时新场景不断涌现，也会催生新的 Harness 实践

## 18.3 AI 工程的三次重心转移

Harness 不是凭空出现的新概念，而是 AI 工程演进中"被逼出来"的结果。过去两年 AI 工程的重心换了三次，每一步都是在上一层**撞到天花板之后**，继续往前啃出来的：

| 阶段 | 解决的核心问题 | 本质 |
| --- | --- | --- |
| **Prompt Engineering 提示词工程** | 怎么让模型"听懂"你想干啥 | 意图的表达——塑造模型的概率空间 |
| **Context Engineering 上下文工程** | 怎么让模型"知道"该用什么信息 | 信息的供给——动态管理上下文 |
| **Harness Engineering 驾驭工程** | 怎么让模型在真实执行中"持续做对"一连串的事 | 执行过程的驾驭——设计包裹模型的完整运行环境 |

### 18.3.1 第一阶段：Prompt Engineering，让模型"听懂"

- **底层原理**：LLM 本质上是**对上下文极度敏感的概率生成器**——它在"预测下一个字最可能是啥"，而不是"思考"。输入越模糊，它猜的范围越大；输入越具体，猜的范围越收敛。所以写提示词的实质是**塑造模型的概率空间**：每一句话都在把输出往某个方向推
- **提示词配方（正式提示词的构成）**：
  1. **角色设定**：你是谁（如"你是一个有十年经验的后端工程师"）
  2. **背景信息**：当前在干啥（如"我正在用 Go 写一个订单系统"）
  3. **参考资料**：相关文档、历史对话、代码片段
  4. **明确任务**：具体要你干啥
  5. **约束条件**：哪些不能做（如"不要修改其他文件"）
  6. **输出格式**：以什么形式返回（JSON、Markdown 还是表格）
- **解决的核心问题**：**模型不是不会，而是你没把问题说明白**。在大模型刚火起来时，任务都是"短链路"（聊天、写文案、翻译、问答，一句问一句答），提示词写好了基本就搞定
- **天花板（为什么不够用了）**：任务变复杂后，**提示词再漂亮，也替代不了"事实"本身**：
  - 让模型"分析我们公司去年的财报"——它没看过财报，分析不了
  - 让模型"按公司内部代码规范写一个新功能"——它没看过规范，不知道怎么遵守
  - 让模型"在我们已有的几十个 API 之间做调用组合"——它根本不知道有哪些 API
- **本质局限**：提示词工程解决的是"**表达**"问题，不是"**信息**"问题——它擅长把任务讲清楚、把约束说明白，但不擅长**凭空补出模型不知道的知识**、**管理一大堆动态变化的信息**、**在长链路任务里维持稳定的状态**

### 18.3.2 第二阶段：Context Engineering，让模型"知道"

- **为什么火**：产品形态变了——从"问一句答一句、链路短、状态少"的聊天机器人，变成要**进真实环境干活**的 Agent：要多轮对话、调用工具、写代码、查数据库、在多个步骤间传递中间结果、根据外部反馈不断修正计划
- **具体示例**：不是简单问"帮我总结这篇文章"，而是让模型完成一个真实任务——"帮我分析这份需求文档、找出潜在风险、结合我们之前几次评审的意见、给出改进建议、最后生成一份发给产品经理的反馈稿"。光靠一句提示词根本完成不了，模型至少需要拿到：
  1. 当前的需求文档
  2. 历史的评审记录
  3. 公司的相关规范
  4. 当前任务的具体目标
...
- **核心定义**：**上下文（Context）= 每一次发给大模型的所有信息的总和**；提示词只是其中很小一部分，还包括用户输入、历史对话、检索资料、工具返回结果、任务状态、中间产物、系统规则、安全约束等
- **核心思想**：**模型未必知道，所以系统必须在合适的时机，把正确的信息送进去**
- **现实约束（为什么不能一股脑全塞）**：
  - **上下文窗口有限**：等于大模型的"短期记忆容量"，再聪明的模型一次也只能记住这么多
  - **上下文腐化（context rot）**：窗口塞太满时，模型开始记不住前面的内容、前后矛盾、忽略最初定下的规则——像被信息淹没的人，抓不住重点
- **实践三步**：
  1. **召回**：从一大堆资料里找出与任务最相关的部分（RAG——把文档切片转成向量入库，每次有问题先到向量库搜出最相关的几块）
  2. **压缩**：信息太多就压缩（先让模型对每段做摘要，只把摘要送进最终上下文；对历史对话，把太老的压成一句话总结，只保留最近几轮原文）
  3. **组装**：按顺序、按格式拼装（**大模型对"靠后的信息"更敏感**，所以重要的指令、当前任务要放在靠后的位置）
- **进阶：渐进式披露（Progressive Disclosure）**——上下文优化的本质不是"给得更多"，而是**"按需给、分层给、在正确的时机给"**：一开始只给模型看每个能力的"目录"（我有这些工具、这些 SOP），等它判断"现在需要用某个工具"时，再把详细说明、参数定义、使用示例动态加载进来（Agent Skills 的核心思路）
- **天花板（为什么还不够）**：提示词没毛病、上下文也没毛病，但**信息都给对了，模型在长链路任务里还是会跑偏**——因为这两个阶段都停留在"**输入侧**"

### 18.3.3 第三阶段：Harness Engineering，让模型"做对"

- **为什么出现**：当模型从"回答问题"走向"连续行动"，出现一个全新的问题——**谁来监督它？谁来约束它？谁在它跑偏的时候把它拉回来？**
- **典型问题示例**（单步表现好，长链路就出问题）：
  - 计划做得很好，但执行的时候突然跑偏
  - 工具调用对了，但理解错了工具返回的结果
  - 在很长的任务链里已经悄悄偏离初衷，但系统完全没有察觉
  - 跑着跑着突然忘了自己最初要干啥
- **词源**：Harness 直译为"**马具 / 缰绳**"——马本身有强大力量，但没有缰绳和马具，这股力量就是失控的（可能往悬崖上跑、把骑手甩下来）。马具的作用，是**让这股力量为你所用**
- **核心公式**：**Agent = Model + Harness**（或 **Harness = Agent − Model**）——除了模型本身之外，几乎所有决定 Agent 能否稳定交付的东西（监督、约束、纠正、验收）都属于 Harness
- **与前两阶段的关系**：不是替代，而是**层层包含**（见下）

### 18.3.4 三层的关系与关键理解

- **三者不是替代关系，而是层层包含**：Prompt 是对"指令"的工程化，Context 是对"输入环境"的工程化，Harness 是对"整个运行系统"的工程化。**Prompt 是 Context 的一部分，Context 是 Harness 的一部分**；做 Harness 时一定包含 Context 工程，Context 工程里又一定包含 Prompt 工程
- **难度与工作量层层放大**：把一句话讲明白不难；把整套信息在合适时机送给模型就开始有门槛；搭出能让模型"持续稳定交付"的运行环境，完全是另一个量级的工程
- **回答了"同样模型为何效果差异巨大"**：差的往往不是模型，而是围绕模型搭起来的那套环境（Harness）是否到位
- **通俗类比（派新人拜访重要客户）**：把任务讲清楚（寒暄、介绍方案、问需求、确认下一步）是 Prompt；把资料准备齐全（客户是谁、之前聊过啥、报价、竞品、会议目标）是 Context；而 checklist、关键节点实时汇报、录音复盘、跑偏及时纠正、按明确标准验收成果这一整套机制，就是 Harness

## 18.4 Harness 的组成：PPAF 闭环

### 18.4.1 PPAF 闭环

一个完备 Agent 系统的核心运行机制，可抽象为一个持续循环的四阶段过程：

1. **感知（Perception）**：获取外部世界状态
2. **规划（Planning）**：基于感知制定行动方案
3. **行动（Action）**：执行工具调用等动作
4. **反思（Feedback / Reflection）**：根据执行结果反馈进行调整

### 18.4.2 二维战略分析矩阵

Harness 的工程化体系正是围绕 PPAF 闭环的四个环节展开，对"感知、规划、行动、反思"逐一进行工程化管理（例如用上下文管理器管理感知、用工具路由管理行动、用反馈汇编器管理反思），共同构成一整套 Agent "马具"。为了理解不同类型 Agent 的能力边界和工程挑战，可构建一个**二维战略分析矩阵**：

- **横轴：AI 认知循环（Cognitive Loop）**
  - **被动响应（React）**：行为主要由外部单次触发驱动，执行预定义的确定性任务，缺乏自主规划和反思
  - **主动规划与反思（Proactive Plan & Reflect）**：基于长期目标自主多步规划、执行、反思和动态调整
- **纵轴：环境系统上下文处理效率（Context Efficiency）**
  - **低效（人工/单点投喂）**：大部分上下文依赖人工手动提供，或只能通过有限的、低效的接口获取
  - **高效（沙盒化/全自动注入）**：运行在高度集成自动化环境中，上下文通过系统级接口（文件系统、API 网关、状态引擎）高效自动捕获和注入

**结论**：Harness 的成熟度，直接决定了 Agent 应用能否从低效、被动的区域，跃迁至高效、主动的区域（即在矩阵中自左下方向右上方移动）。

## 18.5 Harness 的设计

### 18.5.1 顶层抽象：带边界控制的 REPL 容器

架构层面，Harness 的本质可抽象为**一个带边界控制、工具路由与确定性反馈的 REPL（Read-Eval-Print Loop）容器**：它包裹在 LLM 这个非确定性的"大脑"之外，管理从"感知"到"行动"再到"反思"的完整生命周期，将 LLM 的推理能力接入确定性的工程世界：

1. **Read（读取）**：通过**上下文管理器（Context Manager）**，将外部世界（用户输入、API 状态）和内部记忆"翻译"成 LLM 可理解的、高度结构化的 Prompt——实现对"感知"环节的工程化管理
2. **Eval（评估/执行）**：当 LLM 生成一个规划（如 Function Calling）时，通过**调用拦截器（Call Interceptor）**捕获意图，路由到正确的工具执行器；执行过程被严密监控，包括超时、资源配额和错误捕获
3. **Print（打印/反馈）**：工具执行结果（成功数据或异常信息）被**反馈汇编器（Feedback Assembler）**捕获，组装成结构化"观测结果"，重新注入上下文，供 LLM 进行下一轮"反思"与"规划"
4. **Loop（循环）**：上述"读取-评估-打印"不断循环，直到 Agent 达到目标状态或触发终止条件——构成 PPAF 的核心驱动力

### 18.5.2 底层转换机制：在"无限状态"与"有限 Token"间架桥

Agent 的智能涌现建立在对海量状态信息的理解之上；而 Transformer 架构操作的却是**有限的、线性的 Token 序列**。因此 Harness 的核心挑战之一，就是建立高效、可靠的双向映射和转化机制。

#### 上下文管理：从"无限状态"到"有限 Token"

- 上下文（Context）是 Agent 感知的全部来源：任务目标、历史交互、工具定义、当前状态等海量信息
- **规约规则（Reduction Rules）**：上下文管理本质是一系列规约规则的集合——在 Token 预算紧张时，明确哪些信息该被牺牲、哪些被保留
- **注入边界（Injection Boundary）**：定义外部信息（如 RAG 结果）应在 Prompt 的哪个位置（前/中/后）注入，避免"大海捞针（Lost in the Middle）"问题

#### Function Calling：从"文本预测"到"物理执行"

Function Calling（FC）是连接 LLM 规划与物理世界行动的桥梁，包含一个严密且脆弱的生命周期闭环：

1. **Schema 序列化**：将可用工具（函数）列表及参数定义（JSON Schema）序列化为特定格式文本注入 Prompt——这是 LLM 理解其"能力边界"的唯一途径
2. **触发生成**：LLM 在参数空间中进行"模式匹配"，认为某工具能满足当前规划步骤时，生成遵循特定语法的文本（含工具名称和参数值）
3. **确定性反序列化**：Harness 捕获该文本，尝试反序列化为结构化调用请求——**这是最脆弱的环节**（JSON 格式错误、参数类型错误等）
4. **观测注入**：Harness 执行调用，将结果（成功或失败）封装成"观测"文本再次注入 Prompt，完成闭环

**失败面与降级路径**（因 LLM 生成的非确定性，每一步都可能失败）：

- **反序列化失败**：
  - **重试**：向 LLM 提供错误信息（如 "Invalid JSON format"），要求重新生成
  - **回退到文本**：放弃 FC，改由 LLM 生成自然语言指令，交给更传统的解析器处理
- **执行失败**：
  - **交互式补充**：若因参数缺失失败，可向用户请求补充信息
  - **反思与重规划**：将详细错误信息注入上下文，引导 Agent 下一轮反思失败原因并选择其他路径

**核心架构决策：状态分离原则**

- 将 **LLM 严格视为无状态的计算单元（CPU）**；所有需要跨轮次保持一致性的状态（用户会话、任务进度）存储在 Harness 控制的外部上下文状态管理器或其他持久化引擎（内存/硬盘）中
- **反模式**：试图通过 Prompt Engineering 让 LLM 在长对话中自行维护复杂状态，会导致系统行为混乱、不可预测且难以调试

### 18.5.3 三大核心约束与六大设计原则

构建 Harness 时必须直面三大核心约束

![](assets/image-20260810-203703-158.png)

六大设计原则作为应对之道：

1. **为失败而设计（Design for Failure）**：将异常和失败视为系统运行的常态而非个例；所有组件和服务都应具备容错、重试和优雅降级能力
2. **契约优先（Contract-First）**：所有系统内外交互必须由明确的、机器可读的契约（Schema、API、Event）定义——这是实现模块化、可测试性和系统演进的基石
3. **默认安全（Secure by Default）**：安全不是事后添加的功能，而是系统设计的出发点；遵循最小权限、零信任和纵深防御原则
4. **决策与执行分离（Decision vs. Execution）**：将"决定做什么"（规划）与"如何做"（执行）在逻辑和物理上解耦，提升系统的灵活性和可扩展性
5. **万物皆可度量（Everything is Measurable）**：系统的每一个行为、每一次决策、每一次资源消耗都应可度量——没有度量，就没有分析和优化
6. **数据驱动进化（Data-driven Evolution）**：将 Agent 的每一次运行视为一次学习机会；建立从数据采集、标注、回流到模型/知识更新的闭环，是实现系统长期智能增长的唯一路径

## 18.6 Harness 的实现（架构与运行机制）

### 18.6.1 控制平面与数据平面

一个成熟的 Harness 通常拆分为两个平面：

- **控制平面（Control Plane）**：负责"决定做什么"——任务调度、资源配额、行为规划、策略与权限
- **数据平面（Data Plane）**：负责"如何去做"——实际 Agent 运行实例、状态存储、记忆存储、沙盒执行环境

落地时，可把 Harness 看成对现有 AI 环境的一层"智能胶水"：**上接模型 API Gateway，下接沙盒和各类服务**，以工程方式串联各个基建。

### 18.6.2 Agent 核心循环（Observe → Think → Act）

1. **观察（Observe）**：感知当前世界状态——用户输入、工具结果、历史对话、任务进度
2. **思考（Think）**：基于观察信息，由规划器更新目标、拆解任务、选择下一步行动
3. **行动（Act）**：执行内部操作（更新记忆、结束任务）或外部操作（调用工具、发出回复）；行动结果进入下一轮观察

**工程提醒**：核心循环不是简单的 `while(true)`。生产环境中通常需要与工作流引擎或状态机框架集成，支持**暂停/恢复、幂等重试、并发事件处理、长周期任务管理**。

### 18.6.3 记忆分层与 Token 转化

**三层记忆**（生命周期不同，必须分层存储）：

1. **任务状态**：如"当前处理到哪一步、还剩几个、每个的中间打分"——写入进度文件，任务结束归档
2. **会话中间结果**：当前轮次的初步判断——随会话结束即可丢弃，不持久化
3. **长期记忆与用户偏好**：常驻注入，跨所有任务存在（如 CLAUDE.md / .cursorrules 这类规则文件就是"长期记忆"的典型实现）

**Token 转化流水线（Token Transformation Pipeline）**——每轮调用前，把多源信息规约成一个可控的 Prompt：

1. **信息源收集**：聚合用户问题、短期记忆、长期知识检索结果
2. **相关性排序**：基于时间、语义相似度等指标，对候选信息打分
3. **压缩与摘要**：对冗长低密度内容做摘要或结构化提炼
4. **预算分配**：按预设 Token 预算，为不同信息类别分配额度
5. **模板组装**：用结构化模板（显式标注 `[user_request]`、`[tool_output]` 等）拼装最终 Prompt

**关键思想**：**把注意力管理变成一个外部工程问题**——与其指望模型"自己想清楚该关注什么"，不如通过 Token 转化机制主动构建上下文，把有限的窗口留给真正重要的信息。

### 18.6.4 规划模式与执行策略

- **ReAct（Reasoning + Acting）**：思考 → 行动 → 观察 → 再思考，Agent 最基础的循环结构（Agent 本质可看作一个 for 循环）
- **Plan-and-Execute**：先规划完整计划再执行，适合长链路任务
- **Reflexion**：每次失败后让 Agent 反思再重试
- **Tree of Thoughts**：同时探索多条思路再选最好的

**实践建议**：**默认使用 Plan-and-Execute，按需叠加重规划与多 Agent**。大多数企业级场景，用结构化计划配合"异常触发重规划"已足够稳健；只有特别开放、长期的任务，才引入多层规划与多 Agent 协作。

### 18.6.5 沙盒执行框架（四级隔离）

为了让 Agent 可以"动手做事"而不破坏系统，需要给它一个安全的环境独立运行：

| 级别 | 隔离技术 | 特点 | 适用场景 |
| --- | --- | --- | --- |
| Level 1 进程级隔离 | chroot / Linux namespaces / seccomp-bpf | 启动快但仍共享内核 | 可信内部工具 |
| Level 2 容器级隔离 | Docker / containerd | 生态成熟 | **大多数工具执行的默认选择** |
| Level 3 轻量级虚拟机 | Firecracker | 提供独立虚拟内核 | 多租户或执行不可信代码 |
| Level 4 完整虚拟机 | KVM / QEMU | 安全性最高但成本最大 | 极少数特殊任务 |

**推荐策略**：默认采用容器级隔离（Level 2），配合严格系统安全内核与只读根文件系统；对不可信代码或高敏感数据，引入轻量级虚拟机（Level 3）作为加强版沙盒。
...
- **熔断机制**：当某个依赖连续失败时暂时熔断，防止级联故障
- **优雅降级**：关键能力不可用时，自动降级为"弱但安全"的模式（如从"可执行代码"退回"只读 + 建议"）

### 18.6.7 策略门控（安全与合规）

除沙盒外，Harness 还需要一个位于"规划器 → 执行层"之间的**策略门控（Policy Gateway）**，在每次行动前做最后的安全与合规检查：
...
有效合理的评测用来衡量 Agent 系统是否"跑在正确的轨道上"：

- **任务效能（Task Effectiveness）**：任务成功率、指令遵循度、工具使用有效性
- **服务质量（Quality of Service）**：端到端延迟、首次响应延迟、错误率
- **资源效率（Resource Efficiency）**：平均 Token 消耗、平均工具调用次数
- **安全与合规（Security & Compliance）**：策略拒绝率、安全事件数

**指标反向驱动演进**：任务成功率上不去 → 回到规划器和上下文策略排查；错误率和成本居高不下 → 反查沙盒、资源配额和熔断策略是否合理。

## 18.7 成熟 Harness 的六层核心组件

OpenAI、Anthropic、LangChain 等团队产品形态、技术栈、服务对象各不相同，但拆开其 Harness，内部结构惊人地相似——因为"让 Agent 在真实世界稳定工作"这个命题，天然会推着所有人收敛到同一结构。六层组件按"在干啥"分成三组：

- **输入侧（让模型看到正确的东西）**：上下文精细化管理 + 记忆与状态管理
- **动作侧（让模型做出正确的事）**：工具系统 + 任务执行编排
- **校验侧（让模型知道做没做对、出错能爬起来）**：评估观测 + 约束恢复

对应工程师在真实环境干活的三组必要条件：**看得准 → 做得对 → 错了能兜底**。每层解决的一句话问题：

| 层 | 解决的一句话问题 | 核心要点 |
| --- | --- | --- |
| 上下文精细化管理 | 模型这一轮该看到什么？这一轮发给模型的那一坨上下文，长啥样、装了些啥、怎么排布 | 把**角色与目标钉死**；**动态筛选**（just-in-time retrieval）而非一次塞满；**结构化组织**（固定规则/动态证据/中间结论分开），避免"自我污染"；对抗"上下文腐化（context rot）" |
| 工具系统 | 模型用什么动手？ | 回答三个问题：**给哪些工具**（克制，非越多越好）、**何时用哪个**（该查才查）、**工具结果如何喂回**（提炼后回注，避免污染）；MCP（Model Context Protocol）本质是工具层标准化 |
| 任务执行编排 | 模型下一步该干啥？ | 给模型一条**明确的工作轨道**（清晰的步骤序列），解决"每步都会做、串起来就不会"的问题 |
| 记忆与状态管理 | 模型跨轮该记住什么？上一轮发生过的事情，怎么流动到下一轮 | **状态外化到文件系统**（如 claude-progress.txt、init.sh、git history），下一轮干净窗口读文件即可恢复进度；任务状态/会话中间结果/长期记忆分层。没有状态管理的 Agent，每一轮调用之间都是失忆的。 |
| 评估与观测 | 模型做得好不好有没有尺子？ | **Eval 集**（手写典型任务+标注正确答案，每次改动后重跑对比成功率）+ **Trace/日志/指标**（LangSmith、Langfuse 类 trace 系统），让调试从"猜"变成"看"。定位失败那一步发生了什么，才能往 Harness 里补上对应的修复。 |
| 约束与恢复 | 模型出错了能不能爬起来？ **失败不是例外，是常态。** | **约束**（硬编码到代码/linter 规则而非提示词）、**校验**（输出前后自动检查）、**恢复**（每种典型失败有明确恢复路径） |

**使用方式**：六层不是"必须一次搭完"的任务清单，而是一张**路标**——Agent 每次犯错，修复应落到对应层：
...
除了 ReAct 之外，还有几个业界常见的编排模式,不同场景会用不同的编排策略：

- Plan-and-Execute（先规划完整计划再执行，适合长链路任务）
- Reflexion（每次失败都让 Agent 反思一下再重试）、
- Tree of Thoughts（同时探索多条思路再选最好的）。

## 18.8 五大工程难题与设计原则

### 18.8.1 难题一：长链路跑久会失焦 → 重启胜过修补

- **现象**：Agent 一开始目标清晰，跑着跑着开始"忘"——忘初始目标、忘已做决定、重复劳动、偏离主线
- **深层机制：上下文焦虑（Context Anxiety）**——当模型觉得上下文窗口快用完时，不仅丢细节，还会着急收尾（突然简化方案、跳过验证、急于宣布"任务完成"）；且模型对"还剩多少上下文"的估计很不准
- **常见误区（Context Compaction 的局限）**：只把历史压成摘要腾空间，但"已经累了"的负担感还在，Agent 仍会慢慢失焦
- **正解：Context Reset（上下文重置）**——直接把旧上下文窗口整个丢掉，换一个干净窗口接手；**状态全部外化到文件系统**（progress 日志、启动脚本、git history），每轮从干净窗口启动、靠读取"交接文档"恢复进度
- **类比**：像处理内存泄漏——不拼命优化内存，而是重启进程、从磁盘恢复状态
- **原则一：重启胜过修补，状态沉到文件里，Agent 随时可以在干净的上下文窗口里接力继续**

### 18.8.2 难题二：让 Agent 自评偏乐观 → 生产与验收分离

- **现象**：让 Agent 干活后自己评估，尤其在无标准答案的任务（设计 UI、写文案、评估代码可读性）上，自评偏差明显，永远觉得自己干得不错
- **正解：三角色分工（Planner / Generator / Evaluator）**
  - **Planner（规划者）**：把模糊需求扩展成完整规格说明
  - **Generator（生成者）**：一步一步去实现
  - **Evaluator（验收者）**：像 QA 一样真实测试——必须实际操作页面、看具体交互、检查运行结果，验收有"真实环境"托底，而非抽象 review
- 三个角色足够独立，才能形成真正有效的闭环：**规划 → 生成 → 验收 → 修复 → 再验收**
- **原则二：生产和验收必须分离，验收方必须能摸到真实世界**（不要让 Agent 既当运动员又当裁判、既当厨师又当食客）

### 18.8.3 难题三：Agent 反复失败，工程师该干啥 → 补环境而非催模型

- **常见错误反应**：再调调提示词、换个更强的模型——这两个本能反应都是错的方向
- **正解（OpenAI Codex 实践）**：人类工程师几乎不写代码，工作重心压在三件事：
  1. 把产品目标**拆解成 Agent 能力边界内的小任务**，确保每件事都是 Agent 接得住的
  2. Agent 反复失败时，不是催它"再努力一点"，而是看**环境里缺了什么能力**，把能力补进环境
  3. 建立**反馈链路**，让 Agent 真正能看到自己工作的结果
- **关键转变**：遇到 bug 传统做法是加一句提示词"请仔细检查不要有 bug"然后祈祷；正确做法是给 Agent 接上 lint、单测、运行环境，让它自己写完自己跑、看见 bug 自己改——前者是**求模型发挥**，后者是**改造环境。**工程师的价值 **不再是「我一天能写多少行代码」** ，而是 **「我能为 Agent 设计多好的一套运行环境」** 。这才是未来程序员真正的技术含量所在。
- **原则三：Agent 反复失败时，别问模型能不能更努力，要问环境还缺什么**

### 18.8.4 难题四：规范文件越长，Agent 越糊涂 → 少即是多

- **现象（OpenAI 亲历）**：早期把全部规范塞进一个巨大的 AGENTS.md，Agent 反而更糊涂——该文件被当作系统提示词每次全量注入，越长注意力越被稀释（**上下文腐化**在规则文件场景的具体表现）
- **正解：从"百科全书"改为"目录页"**
  - 主文件只保留约 **100 行**核心索引，不告诉 Agent 每条细节规则，只告诉它"想看什么、去哪儿看"
  - 详细内容拆到主题子文档（架构、设计原则、产品规格、执行计划、质量评分各一份）
  - Agent 平时只看目录，真正需要某部分时才钻进对应子文档——**渐进式披露（Progressive Disclosure）**
- **原则四：规则文件宁缺毋滥，给模型看的东西少即是多**（与"按需加载 / 懒加载"一脉相承）

### 18.8.5 难题五：AI slop 技术债越堆越烂 → 规则固化 + 自动偿还

- **现象**：Agent 写绝大多数代码后， **会疯狂模仿仓库已有模式，好的被复制、坏的也被复制；** 早期某段代码写歪，会把歪写法当"惯例"越堆越多，代码库开始"腐烂"（AI slop）
- **失败方案**：靠人工清理（每周周五集中打扫）——Agent 产出速度太快，人工清理根本跟不上，周五清一天周一又堆满（人力追不上机器）
- **正解（两步）**：
  1. 把人类工程师关于"什么是好代码"的隐性经验写成 **Golden Principles（黄金原则）**沉进仓库（如"优先用共享工具包而非手写 helper"）
  2. 让一批后台 Agent 按固定节奏自动扫描仓库、对比原则、找出偏离，自动开修复 PR；大部分修复 PR 可在 1 分钟内被人类审完、直接 auto merge
- **原则五：技术债就像一笔高利息贷款，几乎永远应该每天小额还一点，而不是攒着等某一天集中还**

### 18.8.6 反直觉发现：Agent 用"老技术"反而更稳

- OpenAI 实践中发现 Agent 对那些被人类称为"boring"的老技术掌握得最好，原因三个：**组合性好、API 稳定、训练数据里出现得多**
- 老库在训练数据中示例多（代码示例、StackOverflow 问答、博客），Agent 掌握更牢；刚出的新框架训练样本少，容易张冠李戴
- 极端情况：宁愿让 Agent 自己实现一个小工具函数，也不引入流行 npm 包——自己写的代码 Agent 能 100% 理解和控制，第三方包藏着黑盒行为
- **实践启发**：做 Agent 项目选技术栈不追新，越老、文档越全、在开源社区沉淀越久的技术，Agent 越容易做对

**五大原则口诀**：重启胜过修补；生产验收分家；与其催模型不如改环境；规则宁缺毋滥；技术债天天还。

## 18.9 工程实践要点（从落地经验提炼）

针对"对付 AI 的不确定性，堆 prompt 是负债，做框架才是资产"：

- **本质区别**：prompt 是一次性的说服，harness 是结构性的约束——**模型供给智商，harness 供给纪律**
- **Agent"遗忘"的三重根因**（也是 Harness 设计的着力点）：
  1. **压缩丢失**：上下文压缩（Auto-Compact）省略"看似不重要"的流程步骤
  2. **检索失败**：记忆文件在，但没被加载进上下文
  3. **指令遵循失败**：信息都在，但模型仍然跳步
  - 对应 Harness 三层设计逐一堵漏：**规则外置、状态持久化、门禁阻断**
- **把上下文当预算来管理**：分层加载——常驻层极小（角色、偏好、触发规则），深度内容按需读取；分层标准不是"按功能分类"，而是"按何时被读取"
- **流程强制执行必须外置到确定性基础设施**：不能依赖模型"记住"该执行哪个步骤；门禁必须是确定性代码（eval 式硬校验、hook 拦截），独立于上下文窗口，**fail-closed（默认拒绝，只放行显式允许的操作）**——不管 AI 听不听话，越界就被拦住
- **评测驱动演进**：把 Harness 本身当被测对象，用**确定性评分**（可复现、不漂移）衡量每次改动"变好还是变坏"；宁可要可复现的"粗糙分"，不要会漂移的"精准分"（如 LLM 评委偶尔波动 ±5 分会让 A/B 对比失去意义）
- **演进本质**：从"用更多的字约束 AI"到"用更好的结构约束 AI"；每一阶段的切换往往不是优化，而是止损

## 18.10 面试总结

回答「Harness Engineering」这道题，按四个层次组织：

**第一层：定义与起源**——Harness Engineering 是除了 LLM 模型本身之外，让 Agent 真正能干活的一切基础设施，本质是**优化模型运行环境、机制与基础设施的总和**。核心等式 **Agent = Model + Harness**。起源：Mitchell Hashimoto 提出，OpenAI 报告推广；它不是新发明，而是对现有工程实践的系统性总结与命名。

**第二层：与 Prompt / Context Engineering 的关系**——AI 工程重心换过三次：Prompt 解决"让模型听懂"（意图的表达），Context 解决"让模型知道该用什么信息"（信息的供给），Harness 解决"让模型在真实执行中持续做对"（执行过程的驾驭）。三者**层层包含**：Prompt ⊂ Context ⊂ Harness。难度与工作量逐层放大。

**第三层：为什么需要（R.E.S.T）**——从"玩具"到"可靠工具"需满足四个目标：可靠性（失败可恢复、幂等、行为一致）、效率（资源可控、低延迟、高吞吐）、安全性（最小权限、沙盒、输入输出过滤）、可观测性（全链路追踪、决策可解释、状态可审计）。加上 Agent-First 时代工程师从"执行者"到"设计者"的角色跃迁。

**第四层：怎么做（加分项）**——按"设计 + 实现 + 组件 + 难点"四块讲：

- **设计**：顶层抽象为带边界控制的 REPL 容器（Read 上下文管理器 / Eval 调用拦截器 / Print 反馈汇编器 / Loop 循环）；底层在"无限状态"与"有限 Token"间架桥（规约规则、注入边界、Function Calling 生命周期、**状态分离原则**——LLM 视为无状态 CPU、状态外置）；遵循六大设计原则
- **实现**：控制平面（决定做什么）与数据平面（如何做）；Agent 核心循环 Observe-Think-Act；三层记忆 + Token 转化流水线；四级沙盒隔离；资源管理（预算/超时/重试/熔断/降级）；策略门控
- **组件**：六层核心组件分三组——输入侧（上下文精细化、记忆状态）、动作侧（工具系统、执行编排）、校验侧（评估观测、约束恢复）
- **难点**：五大难题与原则——Context Reset（重启胜过修补）、生产验收分离、补环境而非催模型、规则文件少即是多（渐进式披露）、技术债自动偿还（Golden Principles）

**收尾**：模型决定 Agent 的**天花板**，Harness 决定它能否**落地、稳定交付、跑在生产环境**。在模型迭代速度逐渐放缓的今天，Harness 的提升空间可能比想象中大得多。

# 19. Loop循环工程

**Loop Engineering（循环工程）** 是 AI Agent 工程演进的最新阶段（2026 年 6 月起）：**不再由人直接给 agent 写 prompt，而是设计一个系统，让 agent 自己发现任务、执行任务、验证结果、决定下一步，人不需要出现在循环里**。你的角色从"prompt 的作者"转变为 **prompt 生产系统的设计师**。

## 19.1 定义与核心思想

### 19.1.1 提出背景与关键人物

- **Addy Osmani**（Google Chrome 工程负责人）随后发文正式命名 **Loop Engineering**，并对四代范式做了定位对比：
  - **Prompt Engineering** 问"我该怎么问"
  - **Context Engineering** 问"模型需要知道什么"
  - **Harness Engineering** 问"如何让 Agent 可靠运行"
  - **Loop Engineering** 问"我应该构建什么系统，让 Agent 自己找到工作、完成工作、验证结果——而我根本不需要在循环里"
- **Boris Cherny**（Claude Code 负责人）："我的工作已经不是写提示词了，我的工作是写循环。"

### 19.1.2 核心转变：从"写 prompt"到"设计系统"

- **旧协作方式（回合制）**：你写一条 prompt → 读它的输出 → 再写下一条，全程握着 agent，一回合都不能松手。
- **新协作方式（循环制）**：把"发现任务、布置任务、检查结果、决定下一步"这一整套流程设计成一个能自己运转的循环，让循环去握着 agent。类比工单系统：请求怎么分流、哪类问题转给谁、办结标准是什么、办不了的怎么升级给人，规则定好，系统自己转，不再亲自处理每个请求。
- **杠杆支点移动**：
  - 写一条好 prompt，收益是"**这一次**回答变好"——投入是消耗品
  - 设计一个好 loop，收益是"**之后每一次**循环都变好"——投入是资产
  - 但设计 loop 比写 prompt 难得多：要考虑触发、并行、验证、状态、止损，相当于从"说一句话"升级到"设计一套制度"。**杠杆变长了，对握杠杆的人要求也更高了**

## 19.2 演进脉络：为什么会出现 Loop Engineering

### 19.2.1 四个 Engineering：同一场"瓶颈迁移"

四个范式**不是四种不同技术，而是同一个东西"控制面外推"的四个阶段**：每一层都旨在解决上一层的盲区，但每一层也都会暴露新的盲区。规律是——**模型每变强一截，瓶颈就往外移一层**：

| 阶段 | 时间 | 解决的核心问题 | 控制面 | 暴露的新盲区 |
| --- | --- | --- | --- | --- |
| **Prompt Engineering** | 2022–2024 | 怎么问（表达） | 消息层 | prompt 是静态文档，控制不了模型"能看到什么"；信息是动态的，写进去的知识在其写入那一刻就是上限 |
| **Context Engineering** | 2025 | 喂什么（信息供给） | 会话层 | 看到正确信息 ≠ 做对决策；模型可能"觉得自己看懂了"就行动，缺行动约束 |
| **Harness Engineering** | 2026 Q1–Q2 | 怎么做（执行约束） | 系统层 | 能约束执行，但不能主动调度——不会决定"什么时候该停" |
| **Loop Engineering** | 2026.6 起 | 何时做、何时停（时间调度） | 时间层 | 依赖一个前提：你要能写清楚那个可机器判定的目标条件 |

各层关键知识点补充：

- **Prompt Engineering**：本质是调整条件概率分布——LLM 是函数 P(下一个 token | 前文)，你写的每个字都是"条件"而非"指令"。few-shot、CoT、角色设定都在收窄采样空间。**天花板**：prompt 是静态文档，模型无法感知信息变化（案例：prompt 里引用的退款政策是上个月版本，模型按旧政策算错金额）。
- **Context Engineering**：核心方法是信息架构（RAG、记忆管理、渐进式披露、上下文压缩、工具输出清洗）。核心原则：**无关上下文是负债，不是中性**——不只浪费 token，还会稀释注意力、把输出带向错误方向。**天花板**：控制止步于信息层，控制不了"基于信息做什么决定"。
- **Harness Engineering**：核心公式 **Agent = Model + Harness**；核心发明是 **maker-checker 分离**——用确定性的验证（编译、测试、lint，exit code 是物理事实）锚定概率性的生成（模型的输出）。数据：同一模型不同 Harness 配置解出率相差 **6 倍**。**天花板**：Harness 是质量检测线，不是生产调度系统——它约束每个动作是否"合规"，但不决定"什么时候该停"（$12,847 事故：agent 每一步都合规地烧钱，缺一个叫停机制）。

### 19.2.2 更早的源头：ReAct 与四个结构性顽疾

Loop 的整套设计，是对早期 Agent 架构（ReAct、Plan-and-Execute）结构性缺陷的回应：

- **ReAct（Reasoning + Acting）**：2022 年末提出，核心循环 Thought（思考）→ Action（行动/工具调用）→ Observation（观察）→ 追加到对话历史后重复，直到模型认为任务完成。它第一次把语言模型从"只会回答"推进成"在推理和行动之间循环切换"的系统。
- **Plan-and-Execute**：先由规划器一次性分解出步骤清单，执行器按清单执行，失败或现实不符才触发重新规划。优点是计划执行前可见、可审计、LLM 调用更少；但问题只是从执行阶段挪到了规划阶段——规划器一开始理解错目标，后面执行再精准也没用。

ReAct 在长程、有副作用的真实任务中暴露**四个结构性顽疾**（不是靠"更好的 prompt"能解决的）：

1. **误差累积（Error Compounding）**：假设每步正确率 95%，20 步任务整体成功率 = 0.95²⁰ ≈ 36%（40 步只剩 12%）。步数越长越不可靠，这是概率铁律。
2. **死亡循环（doom-loop）**：调用工具 → 失败 → 观察错误 → 重试几乎同样的调用 → 再失败。上下文堆满"我试了、失败了"反而强化重试倾向，无外部机制打断能跑几百轮（真实事故：运维 agent 遇到陌生错误码，40 分钟重启服务 200 多次，把可控抖动放大成雪崩——缺一个"你试了 200 次了，停下来叫人"的闸门）。
3. **上下文爆炸与污染**：每一步的 Thought、Action、Observation 全部累积进上下文，越长越脏、注意力被稀释、表现越差、产生更多无用 Observation，形成正反馈崩溃螺旋。
4. **没有独立验证闸门**：Observation 只反馈"工具执行的结果"，不反馈"任务是否真的朝目标前进"；缺的不是观察，是判断"观察意味着什么"的独立裁判。

**共同根因**：ReAct 把所有东西——推理、决策、观察、状态——全部塞进同一个上下文窗口，没有隔离、没有分层、没有独立验证、没有外部中断机制。后面几层范式（Prompt → Context → Harness → Loop）本质都在回答同一个问题：**如何让一个概率性的推理引擎，在一个需要确定性的工程环境里，持续产出可靠的结果**。

## 19.3 Loop 的本质：收敛控制系统（不是 while 循环）

很多人以为 Loop Engineering 就是把 agent 丢进一个 while 循环让它一直跑。真正的 Loop Engineering 是**收敛控制系统（Convergence Control System）**：**朝着一个目标自适应地迭代，把上一轮的差距反馈进下一轮，并知道何时该停**。

最小完整形态由**五个部件**组成，缺一不可：

| 部件 | 作用 | 谁来设计 |
| --- | --- | --- |
| **目标/终止条件（GOAL）** | "所有测试通过且覆盖率 > 90%"，**必须能被机器判定**；写不出这个条件就不要上循环 | 你 |
| **驱动（DRIVE）** | 用当前状态构造 prompt，调用一次 Harness（Claude Code 的 /goal、Codex 的 Automations 在做这件事） | 产品原语封装 |
| **执行（HARNESS）** | 工具调用、验证闸门、权限检查——即第三层 Harness 那一整套 | 产品原语封装 |
| **独立裁判（CHECKER）** | 判断目标是否达成；**干活的人（maker）和验收的人（checker）不是同一个** | 你 |
| **安全闸门（GATES）** | 最大迭代次数、最大预算、连续无进展检测——这三样是"烧光预算"事故的解药 | 你 |

**核心结论**：循环体（驱动 + 执行）已经有现成的产品原语帮你做好了。**真正需要你亲自设计的，是目标（GOAL）、独立裁判（CHECKER）、安全闸门（GATES）三个部件**。在 Prompt 时代你设计"怎么问"，Context 时代你设计"能看到什么"，Harness 时代你设计"怎么做"，**Loop 时代你设计的是——做到什么程度算完**。所以：**"你设计的不是循环体，而是终止条件。先写怎么停，再写怎么跑。"** 这是 Loop Engineering 的第一条原则。

### 19.3.1 为什么 CHECKER 必须独立（maker-checker 分离的延续）

- **认知偏差**：同一个模型评估自己的输出时，它知道自己走过的路，会下意识"说服自己"那条路是对的——就像刚写完代码的开发者 review 自己的代码，总觉得"我的代码没问题"。
- **独立 grader 的机制**：系统在任务开始时定义评分标准（rubric），自动创建一个运行在**全新上下文窗口**中的 grader agent；它**只能看到评分标准和最终产物（artifact），看不到原始 agent 的推理路径**，判断不被"这条路是对的"这个先验污染。
- **数据支撑**：独立 grader 比同一模型的自我评估，任务成功率提升约 **10 个百分点**，结构化任务（输出格式明确、评判标准清晰）上提升最大。
- **用到了极致**：连"我做完了"这个停止条件，都不让干活的 agent 自己说——/goal 每轮结束后由一个独立模型判断条件是否达成。

## 19.4 Loop 与 Cron 的区别
...
- **Cron 是开环系统**：按时重复一个固定动作，不看结果、不管对错、每次都一样。
- **Loop 是闭环系统**：结果反馈进下一轮，每一步都比上一步离目标更近。
- 类比：Cron 是定时开关的电暖气（到时间就开就关，不管房间温度）；Loop 是恒温空调（检测当前温度 → 对比目标温度 → 计算差距 → 决定是否继续加热）。

## 19.5 Loop 的组成部分（落地视角：五大件 + 记忆）

把一个真正跑起来的 loop 拆开看，零件是**五大件 + 一份磁盘记忆**，每一件都对应一个"不装它就会翻车"的场景：

1. **自动化（loop 的心跳）**：定时或事件触发（每天早上自动扫 CI 失败、每次 PR 合并自动跑一轮检查）。**没有自动化，loop 就只是一个你跑过一次的任务**。
2. **Worktree（并行隔离）**：多个 agent 同时干活时，防止两个 agent 同时改同一个文件"打架"。git worktree 机制给每个 agent 一个**独立工作目录和独立分支**，共享同一份仓库历史、物理上互不干扰，各干各的、各开各的 PR。
3. **Skill（固化项目知识）**：agent 每个会话都是冷启动，项目的规范、约定、坑一概不知，没解释到的地方它不会空着，会用**自信的猜测**填上。skill 把这些项目知识写成文件放仓库，让 agent 该用的时候自己读。对 loop 的意义：**没有 skill，loop 每个周期都从零重新推导你的项目；有了 skill，知识是复利的**。
4. **Connector（连接外部世界）**：基于 **MCP 协议**的连接器，把 issue 工单、监控、消息、PR 等外部系统接进来。区别：一个 agent 只会告诉你"修复方案在这里"；一个完整 loop 会**自己开好 PR、关联好工单，等 CI 变绿后自己去频道里通知人**。
5. **Sub-agent（写查分离）**：五大件里最有价值的结构性设计——**让写代码的 agent 和检查代码的 agent 分开**。理由：写代码的模型给自己的作业打分时太手下留情；让一个干净上下文的 B 来挑刺，B 没有"希望自己是对的"的包袱，挑出来的问题才是真问题。
6. **记忆（跨会话状态）**：模型在两次运行之间会忘掉一切——今天的循环干了什么、哪些做完了、哪些卡住了，明天的循环一概不知。解法朴素但关键：**把记忆放在磁盘上，而不是上下文里**——一个 markdown 文件、一个任务看板，记录"做完了什么、下一步是什么"，活在单次对话之外。关键句：**"agent 会忘，但 repo 不会。"**

## 19.6 真实示例：晨间 triage loop

Osmani 自用的 loop，任务是：**每天早晨自动把项目里值得修的问题找出来、修好、提交审核**。完整流程：

1. 每天早晨，自动化准时触发，调用一个负责**分诊（triage）** 的 skill
2. skill 读取昨天的 CI 失败记录、未关闭的 issue、最近的提交，把"哪些问题值得处理"写进一个状态文件
3. 对每一个值得处理的问题，开一个**隔离的 worktree**，派一个 sub-agent 进去起草修复
4. 第二个 sub-agent 登场，对照项目的 skill 规范和现有测试，把那份草稿审一遍
5. 审过了，connector 自动开 PR、更新对应的工单
6. loop 搞不定的问题**不硬来**，丢进一个待办收件箱，等真人来看
7. 所有经过写回状态文件，明天早上的循环从今天停下的地方继续

**关键点**：整个过程人只设计了一次，中间任何一步都没有写过 prompt。**人的位置只在两处出现：设计循环的时候，和收件箱里有东西的时候。** 一个好 loop 的标志：你只在设计时出现一次，之后只在收件箱前出现。

## 19.7 工具支持现状

一年前搭一个 loop 意味着写一堆只有自己看得懂、还得永远自己维护的 bash 脚本；现在**五大件基本内置在主流产品里**：
...
- 两家产品的部件几乎一一对应，**loop 的设计正在变得工具无关**——部件是同样的部件，差的只是商标。**值得积累的是"图纸"（loop 的设计），不是对某家工具的肌肉记忆**；图纸画好了，放在哪家产品上都能转。
- 门槛已经从"自己造零件"降到"学会拼装"，剩下的问题只是你想让 loop 替你做什么。

### 19.7.1 /loop 与 /goal 的区别

- **/goal**：跑到你写的条件为真才停（如"目录下所有测试通过且 lint 干净"）。与普通定时循环的区别：定时循环按节奏重复跑（每小时跑一次，跑完就完，对不对另说）；/goal 是**状态/条件驱动**，且**每一轮结束后由一个独立的模型判断条件是否达成**——即"写的人和查的人分开"这条原则，用在了"什么时候算干完"这个停止条件上。
- **/loop**：示例：`/loop 10m 检查当前分支 PR 的 CI 状态：有失败的检查就读日志、修复、推送；全部变绿后停下来，给我一句话总结改了什么`。拆解：
  - `10m` 是**心跳**（间隔）：每 10 分钟自动醒来跑一轮，不用按回车
  - 中间那段是**任务**：每一轮干什么
  - 最后一句是**停止条件 + 汇报**：什么时候算完、完了怎么交差
  - 省略间隔时（`/loop 任务`），节奏由模型自己根据任务耗时决定（比如"CI 一般要跑多久"），不会傻乎乎地一分钟刷一次
- **使用边界**：
  - `/loop` 活在当前会话里，适合"今天盯着这件事"的轮询，**关掉电脑它就停了**；想要睡觉时也在跑的 loop，要用计划任务或云端的 routines，让它脱离你的机器运行
  - 最小 loop 只有心跳、任务、停止条件，**没有 worktree、没有写查分离的 sub-agent**——这不是缺陷，是起点：先让最小的循环转起来，哪天觉得"它自己改的代码我不放心"，再加检查 sub-agent；觉得"想同时盯三个 PR"，再上 worktree。**部件是一件一件长出来的，不是一天配齐的**

## 19.8 Loop 的三个代价（不是 bug，是代价）

Loop 改变了工作，但**没有把人从工作中删除**。有三个问题会随着 loop 越好用而越来越尖锐：

1. **验证仍然归你**：一个无人值守运行的 loop，也是一个无人值守犯错的 loop——你睡觉时它在干活，也意味着你睡觉时它在犯错。就算配了检查 sub-agent 也别高兴太早：**检查 agent 嘴里的 "done" 只是一个声明，不是一个证明**。它说没问题，和真的没问题，中间还隔着你的眼睛。
2. **理解债（Comprehension Debt）**：loop 交付你没写过的代码越快，"仓库里实际存在的东西"和"你脑子里真正理解的东西"之间的鸿沟就越大。它和技术债不同：技术债是代码烂，理解债是**代码可能不烂，但你不知道它为什么是对的**——出问题那天，你面对一片自己"拥有"但不"理解"的代码。顺滑的 loop 不会帮你还这笔债，只会让它涨得更快，**除非你坚持去读 loop 产出的东西**。
3. **认知投降（Cognitive Surrender）**：loop 自己转起来之后，最省力的做法是不再对产出有自己的观点——它给什么就收什么。关键原话：**"带着判断力去设计 loop，它是解药；为了逃避思考去设计 loop，它是助燃剂。同一个动作，相反的结果。"**

此外还有一笔现实的**成本账**：loop 是按循环烧 token 的，sub-agent 每多一个，就多一份模型和工具的开销。务实的花法是把 sub-agent 用在"值得买第二意见"的地方，而不是处处双保险——token 富裕和精打细算这两种人，会设计出两种完全不同的 loop。

## 19.9 面试总结

回答「Loop Engineering」这道题，按四个层次组织：

**第一层：定义**——Loop Engineering 是 2026 年 6 月由 Addy Osmani 提出并命名的概念：把"亲自给 agent 写 prompt 的那个你"替换掉，转而去设计一个让 agent 自己找到工作、完成工作、验证结果、决定下一步的系统，人不需要在循环里。你的岗位从"prompt 的作者"变成"**prompt 生产系统的设计师**"，杠杆的支点从"每一次回答"移到"之后每一次循环"。

**第二层：为什么会出现（演进脉络）**——四个 Engineering 是同一场"瓶颈迁移"：Prompt（怎么问，消息层）→ Context（喂什么，会话层）→ Harness（怎么做，系统层）→ Loop（何时做、何时停，时间层），每一层解决上一层的盲区、又暴露新盲区。更早的源头是 ReAct 的四个结构性顽疾（误差累积、doom-loop、上下文爆炸污染、无独立验证）。Harness 的盲区尤其关键：**它能约束执行，但不能主动调度"什么时候该停"**——这正是 Loop 要补的洞。

**第三层：Loop 的本质（收敛控制系统）**——不是 while 循环，而是**收敛控制系统**：朝目标自适应迭代、把差距反馈进下一轮、知道何时停。五个部件：GOAL（可机器判定的终止条件）、DRIVE（驱动）、HARNESS（执行）、CHECKER（独立裁判）、GATES（安全闸门：最大轮次/预算/无进展检测）。**需要你亲自设计的只有 GOAL、CHECKER、GATES——你设计的是终止条件，不是循环体；先写怎么停，再写怎么跑。** CHECKER 必须独立：独立 grader 用全新上下文窗口、只看产物不看推理路径，成功率比自我评估高约 10 个百分点。与 Cron 的本质区别：**开环 vs 闭环**。

**第四层：落地与代价（加分项）**——五大件 + 磁盘记忆：自动化（心跳）、worktree（并行隔离）、skill（知识复利）、connector（MCP 连接外部世界）、sub-agent（写查分离）、状态文件（跨会话记忆，"agent 会忘，repo 不会"）。工具已内置（Codex / Claude Code 对照，/loop 与 /goal）。三大代价：验证归你（done 只是声明不是证明）、理解债（拥有但不理解）、认知投降（逃避思考的助燃剂），外加按循环烧 token 的成本账。

**收尾**：Loop 把杠杆变长了，但验证、理解债、认知投降这三笔账也同时变大了。**工具分不出你是在加速还是在逃避，你自己分得出**——两个人可以搭一模一样的 loop，得到完全相反的结果：一个用它在自己深刻理解的工作上加速，另一个用它彻底逃避理解工作。
