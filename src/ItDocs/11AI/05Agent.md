...
**关键问题：为什么 Tool 定义必须在初次调用时就传入，不能在对话中途动态增删？** 这是由 **LLM API 的无状态特性 + Function Calling 的机制约束 + Prompt Caching 的工程现实** 三个层面共同决定的：

### 原因一：LLM API 本质上是无状态的

LLM API 没有"会话"概念——每次请求都是独立的，模型不记得上一次请求传了什么工具。所以你必须在**每一轮请求中都全量重发**完整的 tools 列表。不是"首次传一次后面就不用传了"，而是**每轮请求都传**，而且**只能传一个完整的静态快照**。

### 原因二：Function Calling 的"先注册才能调用"硬约束

模型调用工具的本质是：基于本次 API 请求 `tools` 参数里的工具定义（名称 + 描述 + JSON Schema），生成一个符合 schema 的 `tool_use` 结构化输出块。模型**只能调用请求中已注册的工具**——即使凭工具名猜到了要调哪个工具，也无法保证参数名、类型、必填项与注册的 schema 一致，生成不了可被执行的合法调用。

这就形成了**鸡生蛋问题**：要在中途动态添加工具，得先知道要用哪个工具；但模型看不到未加载工具的 schema，就没办法正确发起调用。而让模型主动请求"先加载某个工具"本身，又需要一个已注册的工具来承载这个请求动作。

### 原因三：中途增删工具会导致 Prompt Cache 全部失效

LLM API 的请求结构顺序是：`tools → system → messages`。tools 位于**上下文的最前端**。当启用 Prompt Caching（提示缓存）时，LLM API 会缓存请求的**前缀部分**（通常是 tools + system + 早期 messages），这样后续相同前缀的请求可以复用缓存，大幅降低延迟和成本。

但如果 tools 列表在中途发生变化——哪怕只是增删一个工具定义——整个前缀就变了，**其后的全部缓存都会失效**。这意味着：

- 每一轮都要重新计算 tools + system + 早期 messages 的 KV Cache
- 延迟显著上升（缓存命中时毫秒级，未命中时秒级）
- 成本显著增加（缓存命中时按缓存价格计费，未命中时按全量计费）

所以 Agent 框架的默认策略就是：**会话开始时固定全量注册所有工具，中途不变动**。这是性能和成本的理性选择。

### 有没有解决办法？

有，通过引入**间接层**来绕过"不能中途增删"的限制：

1. **Tool Search / 延迟加载（Deferred Tools）**：上下文只放工具名清单，模型通过一个固定注册的"搜索工具"按需拉取目标工具的完整 schema。Claude Code 的 `ToolSearch` 就是这个思路——用一个永远存在的工具来动态发现其他工具。
2. **分层注册**：只注册少量通用的"路由器"工具，由路由器根据用户请求动态加载子工具。代价是多一轮 API 往返。

但需要理解：**这两种方案并没有真正"中途修改 tools 参数"**，而是通过一个始终存在的中间工具实现了"按需获取工具 schema"的效果。底层的 tools 参数依然是会话开始时固定的那个集合。
