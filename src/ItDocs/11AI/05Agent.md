...
**Loop Engineering（循环工程）** 是 AI Agent 工程演进的最新阶段（2026 年 6 月起）：

**不再由人直接给 agent 写 prompt，而是设计一个系统，让 agent 自己发现任务、执行任务、验证结果、决定下一步，人不需要出现在循环里**。

你的角色从"prompt 的作者"转变为 **prompt 生产系统的设计师**。
