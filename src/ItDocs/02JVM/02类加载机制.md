...
![类加载生命周期](../assets/02JVM/f1b0f86ab95649cdb7c1a1c259fdcd5d.png)
...
![类初始化顺序](../assets/02JVM/0f38e93be6e94c24b06b141fb34e79fc.png)
...
# 12. Java instanceof关键字是如何实现的？

instanceof的使用规则和JVM底层实现原理？

**原理分析**

instanceof用于测试一个对象是否是一个类的实例。

**基本使用规则：**
- obj必须为引用类型，不能是基本类型
- obj为null时返回false
- obj为实际对应类的实例时返回true
- 目标为接口时，obj为其实现类则返回true
- obj为目标类的直接或间接子类，返回true
- 判断依据是new的实际类型而非引用类型声明，父类引用指向子类对象时instanceof子类返回true

**JVM实现原理：**
- 对应JVM的`instanceof`字节码指令
- 执行时从对象实际类开始向上遍历类层级结构，检查每个父类及实现的接口是否匹配目标类型
- 找到匹配返回true，否则false
- 对象为null时指令直接返回false
- 编译器在编译期对明显不可能的类型关系进行静态检查并报错

**常见用例验证：**
- 子类对象的引用，instanceof父类 → true
- 父类引用指向子类对象，instanceof子类 → true
- 同级类互instanceof → false
